...
// گروه عضلانی رو پیدا می‌کنه، سه‌تای برتر رو با همون پسوند ست/تکرار یا
// زمان حرکت اصلی برمی‌گردونه — کاربر خودش انتخاب می‌کنه، نه سوییچ خودکار.
...
// excludeNames: اسم (بدون پسوند ست/تکرار) بقیه‌ی حرکات همون روز — وگرنه
// پیشنهاد ممکنه دقیقا یکی از حرکات از‌قبل‌توی‌برنامه رو برگردونه؛ جایگزینی
// با اون فقط یه هم‌نام تکراری می‌سازه (حرکت قدیمی درست حذف می‌شه ولی یه
// کپی دیگه از یه حرکت دیگه‌ی برنامه به‌جاش میاد)، نه یه جایگزین واقعا جدید.
//
// سه‌مرحله‌ای، کاملا بدون هوش‌مصنوعی — قبلا وقتی این تابع کمتر از max
// نتیجه‌ی «هم‌الگو/هم‌عضله» پیدا می‌کرد، بقیه رو از AI می‌گرفت (کند و
// غیرقابل‌پیش‌بینی برای یه کار صرفا لیستی). حالا اگه مرحله‌ی دقیق کافی
