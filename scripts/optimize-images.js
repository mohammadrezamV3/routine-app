// فشرده‌سازی asset های تصویری ریپو — با `node scripts/optimize-images.js`.
// اجراش idempotent ـه (دوباره اجرا کردنش روی خروجی خودش تقریبا چیزی عوض
// نمی‌کنه)، پس اگه بعدا لوگو/بنر جدیدی اضافه شد فقط کافیه به لیست jobs
// اضافه بشه و یک بار اجرا بشه.
//
// چرا لازم بود: PNGهای اصلی 8-bit RGBA و در ابعاد خیلی بزرگ‌تر از جایی که
// نمایش داده می‌شن ذخیره شده بودن (مثلا logo-icon.png حجم ۴۲۷KB برای
// عنصری که ۳۸px نمایش داده می‌شه). تبدیل به PNG palette + resize به سقف
// منطقی، مجموع ~۱.۳MB رو به ~۱۲۳KB رسوند، بدون افت دیدنی کیفیت
