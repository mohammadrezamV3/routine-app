...
export const metadata: Metadata = {
  ...
  description:
    `${BRAND_FA} (${BRAND_EN}) چیست، چرا ساخته شد و چه بخش‌هایی دارد: روتین روزانه، برنامه‌ی هفتگی، ثبت خواب، ` +
...
// محتوای این صفحه عمداً واقعی و دقیقاً منطبق بر کاری‌ست که اپ الان انجام
// می‌دهد — بدون ادعای اثبات‌نشدنیِ «بهترین» یا آمارِ ساختگی. دلیلِ سئویی‌اش
// هم همین است: گوگل صفحه‌ی نازک (این صفحه قبلاً ۱۹ کلمه بود) را ارزشی
// نمی‌داند، ولی متنِ پُرکلمه‌ی توخالی را هم جریمه می‌کند. متنِ زیر هم برای
// آدم مفید است هم عبارت‌هایی را پوشش می‌دهد که کاربرِ فارسی‌زبان واقعاً
// جست‌وجو می‌کند.
const SECTIONS: { title: string; body: string }[] = [
  {
    ...
    body:
      ...
      `از زندگی‌ات یک اپ جدا نصب کنی — یکی برای برنامه‌ریزی روزانه، یکی برای ثبت خواب، یکی برای برنامه‌ی ورزشی، ` +
  ...
  },
  {
    ...
    body:
      "مشکل اصلی نبودِ ابزار نبود؛ پخش‌بودنِ ابزارها بود. وقتی برنامه‌ی هفتگی‌ات در یک اپ است، تمرین‌هایت در " +
      "اپ دیگر و معاملاتت در یک فایل اکسل، هیچ‌وقت تصویر کاملی از اینکه هفته‌ات چطور گذشته نداری. آریون برای " +
      "همین ساخته شد: یک‌جا ثبت کن، یک‌جا ببین. ضمناً بیشترِ اپ‌های این حوزه انگلیسی‌اند و با تقویم میلادی " +
      "کار می‌کنند، که برای کاربر فارسی‌زبان یعنی هر روز یک اصطکاک کوچک.",
  },
  {
    title: "داخل آریون چه چیزهایی هست؟",
    body:
      "برنامه‌ی روزانه و هفتگی برای ساختن روتین و تیک‌زدن کارها؛ ثبت خواب برای اینکه ببینی ساعت خوابت با " +
      "انرژی روزت چه نسبتی دارد؛ برنامه‌ی بدنسازی با تمرین‌های تفکیک‌شده و ثبت وزن و اندازه‌های بدن؛ " +
  ...
  },
  {
    ...
    body:
      "برای کسی که می‌خواهد روی روتین روزانه‌اش کنترل داشته باشد و پیشرفتش را ببیند، نه صرفاً یک لیست کار " +
  ...
  return (
    <section>
      ...
      <div className="dateline" style={{ marginBottom: 18 }}>
        {`${BRAND_FA} (${BRAND_EN}) — همه‌ی نظمِ زندگی‌ات، یک‌جا`}
      </div>
      ...
      {/* لینکِ داخلی به صفحه‌های عمومیِ دیگر — هم برای کاربر مفید است، هم به
          کراولر کمک می‌کند بقیه‌ی صفحه‌ها را پیدا کند (این سایت هیچ بک‌لینکِ
          بیرونی ندارد، پس لینک‌دهیِ داخلی تنها مسیرِ کشفِ صفحه‌هاست). */}
