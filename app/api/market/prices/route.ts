...
// می‌کنیم. عمدا از سمت سرور صدا زده می‌شه نه با یک اسکریپت شخص‌ثالث توی
...
// سقف تعداد نماد کش‌شده. قبلا این Map هیچ سقف/تخلیه‌ای نداشت و چون این
// روت *بدون لاگین* هم جواب می‌داد، هرکسی می‌تونست با نمادهای ساختگی
...
// نگه‌داشتن سقف کش: اول منقضی‌شده‌ها، بعد قدیمی‌ترین‌ها (Map ترتیب درج رو
...
// نماد بازار: فقط شکل‌های واقعی (AAPL، BTC-USD، ^VIX، EURUSD=X). هرچیز
// دیگه‌ای اصلا به سمت یاهو فرستاده نمی‌شه — نه برای جلوگیری از تزریق (مسیر
// از قبل encodeURIComponent می‌شد) بلکه تا این روت به یه پروکسی عمومی
// دلخواه تبدیل نشه و کشش با نماد آشغال پر نشه.
