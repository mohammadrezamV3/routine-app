 ...
 *
 * هرچیزی که *هر* صفحه‌ی داخلِ اپ موقعِ لود لازم داره، در یک درخواست.
 *
 * چرا لازم شد — با بنچمارکِ ۲۵۰۰ کاربر اندازه‌گیری شد:
 *   • یک لودِ صفحه ۱۲ درخواستِ API می‌زد.
 *   • دیتابیس اصلاً گلوگاه نبود: زیرِ بار ۹ کانکشن idle بود و صفر کوئریِ
 *     منتظر. ولی `/api/health` — که نه به DB دست می‌زنه نه به سشن — از ۳ms
 *     به ۲۳۰ms می‌رفت. یعنی گلوگاه CPU/event loopِ نوده، نه Postgres.
 *   • ظرفیتِ اندازه‌گیری‌شده: بدونِ auth ~۱۲۶۴ req/s، با auth ~۷۶۰ req/s.
 *     یعنی هر درخواست حدودِ ۴۰٪ overheadِ اضافه فقط بابتِ
 *     getServerSession (رمزگشایی و تأییدِ JWT) می‌ده — و این هزینه ۱۲ بار
 *     در هر لودِ صفحه تکرار می‌شد.
 *
 * پس مسئله «کوئریِ کند» نبود، «تعدادِ درخواست» بود. این روت همون داده‌ها رو
 * با **یک** چکِ سشن و **دو** کوئریِ موازی برمی‌گردونه.
 *
 * اندپوینت‌های تکیِ قبلی همگی سرِ جاشون‌ن — هم برای نوشتن، هم برای هر
 * خواندنی که خارج از موجِ لودِ اولیه اتفاق می‌افته.
 */
export async function GET(req: NextRequest) {
  ...
  // بازه اختیاریه: صفحه‌هایی که تاریخچه‌ی روزانه نمی‌خوان (مثلاً /about)
  ...
  // آواتار ستونی از خودِ User ئه، پس با همون کوئریِ حساب میاد — نه یک
  // درخواستِ جدا، که قبلاً بود.
  ...
  for (const r of dailyRows) {
    // همون قالبِ /api/tasks/daily/range تا کلاینت لازم نباشه دو شکل بشناسه
  ...
  // سوپریوزر همیشه همه‌ی ماژول‌ها رو داره — عیناً همون منطقِ /api/account.
  // اگه این‌جا تکرار نشه، سوپریوزری که از مسیرِ bootstrap لود می‌شه دسترسیش
  ...
  return NextResponse.json({
    settings,
    // شکلِ `user` عیناً همونیه که /api/account می‌ده، تا lib/accountCache.ts
    // و ModuleGate بدونِ هیچ تغییری بتونن مصرفش کنن. سوپریوزر هم مثلِ اون‌جا
    // همه‌ی ماژول‌ها رو فعال می‌گیره — منطقِ دسترسی نباید بین دو مسیر فرق کنه.
