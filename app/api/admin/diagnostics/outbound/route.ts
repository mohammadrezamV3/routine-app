 ...
 *
 * چرا لازم شد: دو صفحه‌ی کاملا متفاوت (خرید اشتراک و گزارش هفتگی) با یک
 * پیام یکسان «مشکلی در اتصال به سرور» می‌شکستند، در حالی که هر چیزی که
 * فقط به دیتابیس نیاز داشت درست کار می‌کرد. وجه اشتراکشان این بود که هر
 * دو یک درخواست HTTPS به بیرون می‌زنند (زیبال و گیت‌وی AI).
 *
 * بدون این روت، تشخیص فقط با خواندن لاگ کانتینر ممکن بود. حالا یک بار
 * باز کردن این آدرس دقیقا می‌گوید کدام سرویس در دسترس نیست و چقدر طول
 * می‌کشد — و تفاوت «DNS حل نمی‌شود» با «کند است» با «۴۰۳ می‌دهد» را
 * روشن می‌کند.
 *
 * فقط سوپرادمین: خروجی‌اش نام سرویس‌ها و وضعیت زیرساخت را لو می‌دهد.
 */

// هر تست سقف خودش را دارد تا یک سرویس مرده کل پاسخ را نگه ندارد.
...
function probes(): Probe[] {
  ...
  // یا از بسته‌شدن مسیر خروجی، یا از ردکردن خود پنل (اعتبار/الگو).
  // ارسال عمدا fire-and-forget است، پس هیچ‌کدام در UI دیده نمی‌شود و
  // تنها راه سریع تفکیکشان همین پروب است. این درخواست پیامکی نمی‌فرستد:
  // اندپوینت اعتبار است، نه ارسال.
  list.push(
...
async function runProbe(p: Probe) {
  ...
  try {
    ...
    // برای ملی‌پیامک خود بدنه هم معنی دارد: مسیر شبکه ممکن است باز باشد
    // ولی پنل به‌خاطر نام‌کاربری/رمز غلط یا اتمام اعتبار ارسال نکند —
    // که از بیرون دقیقا شبیه «پیامک نمی‌آید» دیده می‌شود.
    ...
    // هر پاسخ HTTP — حتی ۴۰۱/۴۰۳/۵۰۰ — یعنی مسیر شبکه باز است. فقط
    // نرسیدن (DNS/فایروال/تایم‌اوت) شکست واقعی این تست است.
...
/**
 * تأخیر رفت‌وبرگشت دیتابیس. اگر این عدد بالا باشد، «کندی سایت» از سمت
 * Postgres یا استخر اتصال Prisma است، نه از سرویس‌های بیرونی.
 */
async function dbLatency() {
  const started = Date.now();
  try {
    // عمدا raw SQL نیست (قانون پروژه): یک findFirst محدود به یک ستون و
    // یک ردیف، که فقط روی کلید اصلی می‌رود — عملا هم‌هزینه‌ی ping است.
...
/**
 * لگ حلقه‌ی رویداد: چقدر طول کشید تا یک تایمر صفرثانیه‌ای واقعا اجرا شود.
 * عدد بالا (ده‌ها یا صدها میلی‌ثانیه) یعنی پروسه مشغول/تحت فشار GC است —
 ...
 *
 * چرا لازم شد: سرویس `migrate` در docker-compose پروفایل `tools` دارد و با
 * `docker compose up` معمولی اجرا **نمی‌شود**. بعد از یک دیپلوی که مدل
 * جدید آورده بود، اپ با کلاینت تازه بالا آمد ولی دیتابیس اسکیمای قدیمی
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    // اگر db.ms بالا باشد → مشکل سمت دیتابیس/استخر اتصال.
    // اگر eventLoopLagMs بالا باشد → پروسه تحت فشار CPU/حافظه است.
    // اگر هر دو پایین ولی outbound کند باشد → مشکل شبکه‌ی خروجی سرور است.
    db,
    // اگر ok:false بود، دستور رفعش داخل همین خروجی آمده است.
    schema,
    ...
    // آدرسی که واقعا برای ساخت لینک‌های بازگشت درگاه استفاده می‌شود.
