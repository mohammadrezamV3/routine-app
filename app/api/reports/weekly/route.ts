...
export async function GET(req: NextRequest) {
  ...
  // try/catch لازم است چون `getOrGenerateWeeklyReport` به گیت‌وی بیرونی AI
  // وصل می‌شود. بدون آن، هر خطایی (گیت‌وی در دسترس نیست، کلید تنظیم نشده،
  // تایم‌اوت) باعث می‌شد نکست یک صفحه‌ی **HTML** با کد ۵۰۰ برگرداند نه JSON؛
  // کلاینت هم روی `r.json()` خطا می‌خورد و می‌افتاد توی catch خودش و پیام
  // گمراه‌کننده‌ی «مشکلی در اتصال به سرور» را نشان می‌داد — انگار اینترنت
  // کاربر قطع است، در حالی که مشکل سمت سرور بود.
  try {
    ...
    if (e?.message?.includes("ARVAN_AI")) {
      return NextResponse.json({ error: "سرویس هوش مصنوعی روی این سرور تنظیم نشده" }, { status: 503 });
    }
    if (e?.name === "TimeoutError" || e?.name === "AbortError") {
      return NextResponse.json({ error: "ساخت گزارش بیش از حد طول کشید — چند دقیقه دیگر دوباره امتحان کن" }, { status: 504 });
    }
    return NextResponse.json({ error: "ساخت گزارش هفتگی ناموفق بود — اگر تکرار شد به پشتیبانی اطلاع بده" }, { status: 502 });
  }
}
