...
//
// مرحله‌ی اولِ ورود وقتی دومرحله‌ای روشنه: رمز این‌جا بررسی می‌شه و اگه درست
// بود یک کدِ پیامکی می‌ره. نشستی این‌جا صادر نمی‌شه — صدورِ نشست فقط از
// providerِ «sms-2fa» توی lib/auth.ts انجام می‌شه که خودش کد رو دوباره از
// صفر اعتبارسنجی می‌کنه.
//
// پاسخ عمداً بینِ «رمز غلط» و «کاربر وجود نداره» فرق نمی‌ذاره (همون قرارداد
// ضدِ user-enumerationِ بقیه‌ی اپ): هر دو حالت `{ required: false }` برمی‌گردونن
// تا فرانت مسیرِ عادیِ ورود رو ادامه بده و همون‌جا خطای عمومی بگیره.
export async function POST(req: NextRequest) {
  ...
  // فقط چهار رقمِ آخرِ شماره نشون داده می‌شه، نه کلِ شماره
