...
// PUT /api/notepad/pages/:id/blocks — جایگزینیِ کاملِ بلاک‌های یک صفحه با
// آرایه‌ی ارسالی از کلاینت (اتوسیو همیشه کلِ لیستِ فعلیِ صفحه رو می‌فرسته،
// نه یک بلاکِ تکی — برای Phase 1 خیلی ساده‌تر و مطمئن‌تر از sync
// per-operation‌ه، و چون تعدادِ بلاکِ هر صفحه معمولاً کوچیکه هزینه‌ی واقعی
// نداره). idهایی که توی آرایه‌ی جدید نیستن حذف می‌شن؛ بقیه‌ی idهای موجودِ
// همین صفحه آپدیت می‌شن و idهای تازه create — یعنی id مستقلِ هر بلاک از
// سمتِ کلاینت حفظ می‌مونه (به‌جز موردِ نادرِ collision، پایین توضیح داده شده).
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // یه id که توی این صفحه قبلاً نبوده ممکنه (در عمل تقریباً غیرممکن چون
  // idها سمتِ کلاینت با آنتروپیِ بالا ساخته می‌شن، ولی بازم چک می‌کنیم — طبق
  // قاعده‌ی IDOR-safeِ پروژه) قبلاً متعلق به یه صفحه‌ی دیگه (حتی یه کاربرِ
  // دیگه) باشه؛ چنین idـی رو نباید upsert کنیم چون ممکنه رکوردِ اون صفحه رو
  ...
  // توجه: دو درخواستِ اتوسیوِ هم‌پوشان (مثلاً debounceِ کلاینت با تایپِ سریع
  // پشتِ‌سرِهم چندبار پشتِ‌هم فایر بشه) می‌تونن هم‌زمان به این روت برسن؛ اگه
  ...
  // upsert این race رو در سطحِ دیتابیس اتمیک حل می‌کنه (ON CONFLICT). فقط
  // موردِ نادرِ collisionِ id با صفحه‌ی یه کاربرِ دیگه از این قاعده مستثناست —
  // اونجا عمداً upsert نمی‌کنیم چون ممکنه رکوردِ غریبه رو بازنویسی کنه.
