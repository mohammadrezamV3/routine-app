...
// POST /api/exercise/plan/manual — برنامه‌ای که خودِ کاربر دستی وارد کرده
// (بدونِ هوش مصنوعی/قالبِ آماده)، جایگزینِ /api/exercise/plan (که فقط
// AI-یا-fallback می‌سازه) برای وقتی کاربر برنامه‌ی شخصیِ خودشو داره.
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(planData) || planData.length === 0) {
    return NextResponse.json({ error: "حداقل یک روزِ تمرینی لازم است" }, { status: 400 });
  }
  ...
  for (const d of planData) {
    if (!d || typeof d.day !== "string" || !FA_WEEKDAY.includes(d.day)) {
      return NextResponse.json({ error: "روزِ نامعتبر در برنامه" }, { status: 400 });
    }
    if (seenDays.has(d.day)) {
      return NextResponse.json({ error: `روزِ «${d.day}» بیش از یک بار انتخاب شده` }, { status: 400 });
    }
    seenDays.add(d.day);

    const items = Array.isArray(d.items) ? d.items.map((i) => String(i).trim()).filter(Boolean) : [];
    if (items.length === 0) {
      return NextResponse.json({ error: `برای روزِ «${d.day}» حداقل یک حرکت لازم است` }, { status: 400 });
    }
