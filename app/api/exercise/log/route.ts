...
export async function POST(req: NextRequest) {
  ...
  const { planId, date, completed, completedItems } = body as {
    planId: string; date: string; completed: boolean; completedItems?: string[];
  };
  if (!planId || !date) return NextResponse.json({ error: "planId and date required" }, { status: 400 });

  const items = Array.isArray(completedItems) ? completedItems.filter((x) => typeof x === "string") : undefined;

  await prisma.exerciseLog.upsert({
    where: { userId_planId_date: { userId, planId, date: new Date(date) } },
    create: { userId, planId, date: new Date(date), completed, completedItems: items ?? undefined },
    update: { completed, ...(items !== undefined ? { completedItems: items } : {}) },
  });
