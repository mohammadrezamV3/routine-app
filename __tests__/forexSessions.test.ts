...
// این تست‌ها روی همان چیزی تمرکز دارند که با عدد ثابت UTC می‌شکند: تغییر
// ساعت تابستانی. هر تاریخ عمدا یک‌بار در تابستان و یک‌بار در زمستان انتخاب
// شده تا اگر کسی روزی این محاسبه را به عدد هاردکد UTC برگرداند، تست بیفتد.

describe("wallClockIn", () => {
  it("لحظه‌ی UTC را به ساعت محلی درست تبدیل می‌کند", () => {
...
describe("isSessionOpen — لندن با احتساب DST", () => {
  it("۰۹:۰۰ به وقت لندن در هر دو فصل باز است", () => {
...
describe("isForexOpen", () => {
  ...
  it("جمعه بعد از ۱۷:۰۰ نیویورک بسته و قبلش باز است", () => {
    // ۱۷ ژوئیه ۲۰۲۶ جمعه؛ تابستان نیویورک = UTC-4
...
describe("sessionsAt", () => {
  it("همپوشانی لندن و نیویورک هر دو را برمی‌گرداند", () => {
  ...
  it("آخر هفته هیچ جلسه‌ای باز نیست", () => {
