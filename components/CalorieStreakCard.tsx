...
// هفت روز کامل هفته (درخواست صریح کاربر) — نه شش روز. اولین فرزند DOM
// «امروز»ه، که توی چیدمان RTL یعنی سمت راست؛ پس هفته از راست شروع می‌شه و
...
// «روند موفقیت» — خلاصه‌ی استریک دیگه یه تمبر دایره‌ای بزرگ وسط کارت
// نیست؛ طبق درخواست صریح کاربر همون بالای کارت، سمت چپ (کنار تایتل)
// جا گرفته — فقط آیکون شعله + عدد، بدون هیچ دایره/باکسی دورش (از
// `StreakFlame` مشترک استفاده می‌کنه، هم‌قاعده‌ی استریک دوست‌ها توی
// DashFriendsCard). ردیف دایره‌ی روزها زیرش دست‌نخورده مونده. امروز
...
export function CalorieStreakCard({ rangeEntries, targetKcal, delay }: { rangeEntries: Entry[]; targetKcal: number; delay?: number }) {
  ...
  // امروز باید اولین فرزند DOM باشه — توی چیدمان RTL یعنی سمت راست (دقیقا
  // مثل رفرنس: «ش» امروز و خالیه، سمت راست؛ روزهای قدیمی‌تر به چپ می‌رن)
