...
// import معمولی نوشته شده بود، باندلر کل ماژول exerciseCatalog (~۸۵KB،
// شامل EXERCISE_CATALOG) رو به گراف همین کامپوننت اضافه می‌کرد — و این
// کامپوننت توی داشبورد /exercise همیشه رندر می‌شه. با `import type` اون
// وابستگی موقع کامپایل کاملا حذف می‌شه.
import type { MuscleKey } from "@/lib/exerciseCatalog";

// دیاگرام بدن (نمای جلو + پشت) — یک سیلوئت توپر بدن (سر/گردن/تنه/بازو/
// پا، نه فقط طرح‌واره‌ی خط‌دور قبلی) به‌عنوان زمینه، و روی هر ناحیه یک
// <path> دست‌ساز با منحنی‌های بزیه که شکل واقعی همون عضله رو دنبال
// می‌کنه. وقتی عضله فعاله با گرادیان طلایی روشن می‌شه، وگرنه محو می‌مونه.

type Region = { key: MuscleKey; view: "front" | "back"; d: string };

const REGIONS: Region[] = [
  // ==================== نمای جلو ====================
  // سرشانه (دلتوئید جلویی) — گنبد روی مفصل شونه
  ...
  // سینه — دو لب قطره‌ای که از استخوان جناغ به سمت زیربغل خم می‌شن
  ...
  // جلوبازو — دوکی با پیک عضله در میانه
  ...
  // شکم — سه ردیف دوتایی (سیکس‌پک) با فاصله‌ی وسط برای خط میانی
  ...
  // مایل‌های شکم — نوار زاویه‌دار کنار شکم
  ...
  // چهارسر ران — پهن بالا، باریک نزدیک زانو
  ...
  // ==================== نمای پشت ====================
  // ذوزنقه‌ای (تراپز) — بادکنک پهن روی شونه/گردن
  { key: "traps", view: "back", d: "M60,20 Q78,24 84,34 Q70,44 60,46 Q50,44 36,34 Q42,24 60,20 Z" },
  // پشت (لات‌ها/زیربغل) — بال V شکل که به کمر باریک می‌شه + میان پشت
  ...
  // همسترینگ — قطره‌ی کشیده‌ی پشت ران
  ...
  // ساق پا (پشت/دوقلو) — برجسته‌تر بالا
...
// سیلوئت توپر بدن (سر، گردن، تنه، بازوها، پاها) — هم‌زمینه‌ی هر دو نما،
// چون طرح کلی بدن از جلو/پشت تقریبا یکیه؛ فقط ناحیه‌های روش فرق می‌کنن.
// یه خط دور نازک (هم‌رنگ بک‌گراند کارت) هم داره — دقیقا مثل عکس
// آناتومی مرجع که هر عضله با یه خط باریک از بقیه جدا می‌شه، نه یه سیلوئت
// یک‌دست بدون مرز.
