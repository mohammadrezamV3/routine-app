// آیکون «ساخت با هوش مصنوعی» — دو ستاره‌ی چهارپره‌ی توخالی (بدون پرشدن)
// شبیه لوگوی جمنای: یکی بزرگ (اصلی، پایین‌راست) و یکی کوچیک (بالا‌راست)،
// طلایی‌رنگ با گرادیانت. ستاره‌ی کوچیک به‌طور پیش‌فرض یه انیمیشن درخشیدن
// ملایم پیوسته داره (نه فقط واکنش به هاور/فشار دکمه‌ی والد) — با prop 
...
// `still`: ستاره‌ی کوچیک بی‌حرکت می‌مونه. توی «محاسبه‌ی هوشمند» کالری
// طبق درخواست صریح کاربر باید یک‌جا وایسته، نه اینکه دائم چشمک بزنه.
export function AiSparkleIcon({ size = 22, still = false }: { size?: number; still?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* قبلا translate(22,-1) بود — با اسکیل ۰.۴۵۸۳ (~۱۱ واحد)، از x=۳۳ و
          y=-۱ رد می‌شد، یعنی از لبه‌ی viewBox ۳۲×۳۲ (بالا و راست) بیرون
          می‌زد و کلیپ می‌شد؛ نصفه‌ونیمه دیده می‌شد. حالا کاملا داخل محدوده‌ست. */}
