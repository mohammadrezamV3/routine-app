...
/**
 * دادهٔ لودِ اولیه را **داخلِ خودِ HTML** می‌فرستد، نه با یک درخواستِ جدا.
 *
 * چرا: حتی بعد از این‌که همهٔ درخواست‌ها به یک `/api/bootstrap` جمع شدن، و
 * حتی وقتی تأخیرِ شبکه **صفر** بود، اولین بایتِ داده ۱۳۲ms بعد می‌رسید —
 * چون مرورگر باید اول HTML را پارس کند، بعد اسکریپتِ inline را اجرا کند،
 * بعد یک رفت‌وبرگشتِ کاملِ دیگر برای داده بزند. آن رفت‌وبرگشت ذاتی نیست:
 * سرور همان لحظه‌ای که HTML را می‌سازد، دسترسیِ کامل به همان داده دارد.
 *
 * با inline کردن، دادهٔ صفحه دقیقاً هم‌زمان با HTML می‌رسد — صفر رفت‌وبرگشتِ
 * اضافه، در هر تأخیرِ شبکه‌ای.
 *
 * برای مهمان‌ها هیچ کوئری‌ای نمی‌زند و هیچ چیزی رندر نمی‌کند.
 */
export async function InlineBootstrap() {
  const jar = cookies();
  // کوکیِ راهنما ارزان‌ترین در است: مهمان‌ها اصلاً به رمزگشاییِ JWT هم نمی‌رسند.
  ...
  // تأیید می‌کند** — این‌جا هم مثل هر روتِ دیگر، کوکیِ راهنما هیچ تصمیمِ
  // امنیتی‌ای نمی‌گیرد؛ فقط جلوی کارِ بی‌مورد برای مهمان را می‌گیرد.
  ...
  try {
    ...
    // کارتِ دوستان روی داشبوردها همیشه لود می‌شود، پس همین‌جا می‌آید —
    // همان منطقِ /api/friends، فقط بدونِ رفت‌وبرگشتِ شبکه.
    ...
    // اگر دیتابیس در دسترس نبود، صفحه نباید بشکند — کلاینت خودش مسیرِ
    // عادیِ fetch را می‌رود.
    return null;
  }

  // JSON داخلِ <script type="application/json"> امن است چون مرورگر محتوایش
  // را اجرا نمی‌کند؛ تنها فرارِ لازم `<` است تا رشته‌ای مثل `</script>` تگ را
  // زودتر نبندد. `JSON.parse` سمتِ کلاینت هم دوباره اعتبارسنجی می‌کند.
