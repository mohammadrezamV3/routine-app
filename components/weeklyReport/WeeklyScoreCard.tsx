...
const CONFIDENCE_LABEL: Record<string, string> = { high: "دقت بالا", medium: "دقت متوسط", low: "دقت کم — داده محدود" };

// کارت اصلی بالای صفحه — امتیاز کلی + تغییر نسبت به هفته‌ی قبل + خلاصه‌ی
// اجرایی AI (یا جای‌گزین صادقانه اگه AI در دسترس نبود، بند ۶۶).
export function WeeklyScoreCard({
  ...
  return (
    <motion.div className="wr-score-card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
      {overallScore == null ? (
        <div className="wr-score-empty">هنوز داده‌ی کافی برای امتیاز این هفته نداریم.</div>
      ) : (
        <>
          ...
          {confidence === "low" && <div className="wr-confidence-note">این تحلیل بر اساس داده‌ی محدود انجام شده است.</div>}
        </>
      )}

      {status === "COLLECTING" && <div className="wr-collecting-note">در حال جمع‌آوری داده‌ی این هفته — گزارش نهایی بعد پایان هفته آماده می‌شه.</div>}

      <div className="wr-ai-summary">
        {aiSummary ? aiSummary : "تحلیل هوشمند برای این هفته در دسترس نیست — آمار هفتگی همچنان کامل و آماده‌ست."}
      </div>
    </motion.div>
  );
}
