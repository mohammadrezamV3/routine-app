...
// اصلا هم‌رنگ بقیه‌ی اپ نبود. این یکی کاملا با استایل خود اپ می‌سازه و
// اسکرولش (مثل بقیه‌ی لیست‌های اپ) بدون اسکرول‌بار قابل‌دیدنه.
...
export function JalaliDatePicker({
  ...
  /** روزهای قبل از امروز رو غیرفعال می‌کنه — برای جاهایی که فقط تاریخ آینده معنی داره (مثلا انتقال برنامه) */
  disablePast?: boolean;
  /** عنوان بالای پاپ‌آپ — وقتی همین کامپوننت پشت‌سرهم برای دو فیلد مختلف
      (مثلا تاریخ شروع بعد پایان) استفاده می‌شه، بدون این عنوان کاربر
      اصلا متوجه نمی‌شه که context عوض شده و داره یه فیلد دیگه رو پر می‌کنه. */
  title?: string;
}) {
  useLockBodyScroll();
  const now = new Date();
