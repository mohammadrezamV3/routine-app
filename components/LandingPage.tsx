...
// نمونه‌ی تصویری «امروز چی داریم» — دیتای واقعی کاربر نیست (این بخش قبل از
// لاگین دیده می‌شه)، فقط یک پیش‌نمایش نمونه از شکل واقعی چک‌لیست روزانه.
const TODAY_ITEMS = [
...
function FeatureCarousel() {
  ...
  // کشیدن به چپ باید عقب ببره (اسلاید قبلی)، کشیدن به راست باید جلو ببره (اسلاید بعدی)
      ...
      <div className={`relative overflow-hidden rounded-[28px] border ${t.cardBorder} ${t.cardBg} ${t.shadow} backdrop-blur-xl`}>
        <AnimatePresence initial={false} custom={dir} mode="popLayout">
          ...
          >
            ...
            {/* دقیقا هم‌کلاس نقطه‌های ویزارد برنامه‌ی هوشمند (AiExercisePlanWizard) —
                همون سایز/رنگ/حالت فعال، این‌بار برای شماره‌ی اسلاید قابلیت‌ها */}
...
function TodayProgressCard({ isIntl }: { isIntl: boolean }) {
  ...
  // پیش‌فرض «همه‌چیز از قبل تیک‌خورده»ست — دقیقا همون چیزی که سرور رندر
  // می‌کنه (بدون ریسک hydration mismatch). با هر اسکرول رو‌به‌پایین که
  // کارت وارد دید بشه، صفر می‌شه و دونه‌دونه دوباره پر می‌شه؛ با اسکرول
  ...
  // این «حالت ریورس» ه، بدون نیاز به دیدن یه انیمیشن معکوس روی چیزی که
  // خودش دیگه بیرون صفحه‌ست.
    ...
    const obs = new IntersectionObserver(([entry]) => {
      ...
      if (entry.isIntersecting) {
        if (dirRef.current === "up") {
          // از پایین صفحه اومده بالا و دوباره به این کارت رسیده — مستقیم
          // حالت کامل رو نشون بده، نیازی به پخش دوباره‌ی انیمیشن نیست.
        ...
        // با اسکرول رو‌به‌بالا از دید خارج شده — ریست بی‌صدا (خود کارت
