...
// فرمِ «پروفایل ورزشی» — قد/وزن/سن/جنسیت. از همان منبعِ مشترکِ
// lib/bodyMetrics.ts می‌خوانَد که فرم‌های بدنسازی/کالری هم باهاش کار
// می‌کنند، پس هرجا یک‌بار پر بشه بقیه‌ی جاها هم از قبل پرن.
//
// کامپوننتِ جداست چون طبقِ درخواستِ صریح، همین فرم داخلِ صفحه‌ی پروفایل
// (زیرِ تایتلِ دومش) می‌نشیند، نه توی یک صفحه‌ی جدا.
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="account-card" style={{ padding: 16 }}>
      ...
      <button type="button" className="account-outline-btn" style={{ marginTop: 16 }} onClick={save} disabled={saving}>
        {saving ? "در حال ذخیره…" : saved ? "ذخیره شد" : "ثبت تغییرات"}
      </button>
    </motion.div>
  );
}
