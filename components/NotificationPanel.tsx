...
async function loadPendingNotifications(): Promise<NotifItem[]> {
  ...
  for (const t of tasks) {
    ...
    if (nowMinutes >= startMinutes - 30 && nowMinutes < startMinutes) {
      items.push({ id: `soon:${t.id}`, title: "یادآوری برنامه", body: `تا ۳۰ دقیقه دیگه وقت «${t.name}» می‌رسه.` });
    } else if (nowMinutes >= startMinutes) {
      items.push({ id: `now:${t.id}`, title: "یادآوری برنامه", body: `وقت «${t.name}» رسیده.` });
    }
  }
        ...
        if (plan) {
          ...
          if (todayPlan) {
            ...
            if (!logData.completed) {
              items.push({ id: "exercise-today", title: "یادآوری تمرین", body: `برنامه‌ی ورزشی امروز (${todayPlan.focus}) هنوز ثبت نشده.` });
            }
          }
        }
      }
