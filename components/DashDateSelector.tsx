...
export function DashDateSelector({
  ...
  //
  // نسخه‌ی قبلی عرضِ هر پیل رو ثابت ۶۲px (موبایل) فرض می‌کرد، ولی پیل‌ها
  // `shrink-0`ـن و عرضشون از محتواشون میاد — «چهارشنبه» + «۳۰ شهریور» روی
  // یه گوشیِ ۳۹۰ پیکسلی حدودِ ۷۵px می‌شه، نه ۶۲. نتیجه این بود که تخمین
  // بیشتر از واقعیت درمی‌اومد، پیل‌ها سرریز می‌کردن و عملاً فقط سه روز
  // دیده می‌شد (بقیه زیرِ لبه‌ی اسکرول). حالا عرضِ واقعیِ رندرشده‌ی
  // پهن‌ترین پیل اندازه گرفته می‌شه. چون خودِ پیل‌ها با تغییرِ تعداد
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      const gap = 6;
      // پدینگِ افقیِ خودِ نوار (px-3) داخلِ clientWidth حساب شده، پس ازش کم می‌شه
      const usable = w - 24;
  ...
  // (هم‌جهت با فلش‌های دسکتاپ که توی RTL «قبلی» سمتِ راسته). فقط وقتی
  // حرکتِ افقی واضحاً از عمودی بیشتره عمل می‌کنه تا اسکرولِ عمودیِ صفحه
  // رو نشکنه.
