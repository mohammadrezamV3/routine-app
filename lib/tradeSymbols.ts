// اندازه‌ی قرارداد نمادها — فقط و فقط برای **پیشنهاد** محاسبه‌ی سود/زیان و
// ریسک در فرم ثبت معامله.
//
// چرا این فایل لازم شد: فرمول قبلی ژورنال `(خروج − ورود) × لات` بود که
...
//
// نکته‌ی مهم محصولی: هیچ‌کدام از این اعداد «حقیقت نهایی» نیستند — هر بروکر
// اندازه‌ی قرارداد کمی متفاوت دارد. برای همین این‌ها فقط یک عدد پیشنهادی
// می‌سازند که کاربر می‌تواند بازنویسی‌اش کند؛ چیزی که ذخیره می‌شود همان
// عدد تأییدشده‌ی کاربر است، نه خروجی این جدول.

import { TRADE_PAIRS, TradePair } from "./tradePairs";

export type SymbolKind = "FOREX" | "METAL" | "ENERGY" | "INDEX" | "CRYPTO";

/** واحد در هر «لات استاندارد» */
const CONTRACT_SIZE: Record<string, number> = {
  // فارکس — لات استاندارد ۱۰۰٬۰۰۰ واحد ارز پایه
  __FOREX_DEFAULT__: 100_000,
  // فلزات
  XAUUSD: 100, XAGUSD: 5_000, XPTUSD: 100, XPDUSD: 100,
  // انرژی
  USOIL: 1_000, UKOIL: 1_000, NATGAS: 10_000,
  // شاخص‌ها و کریپتو — معمولا «۱ واحد به‌ازای هر لات»
  __INDEX_DEFAULT__: 1,
...
/** اندازه‌ی قرارداد؛ برای نماد ناشناخته ۱ برمی‌گردد (یعنی «همان عددی که وارد کردی») */
...
/**
 * پیشنهاد سود/زیان به ارز حساب.
 * فقط وقتی عدد می‌دهد که هر سه‌ی ورود/خروج/حجم موجود باشند — وگرنه null،
 * چون «صفر» با «نمی‌دانم» یکی نیست و کاربر ژورنال باید تفاوتش را ببیند.
 * کمیسیون و سواپ (اگر وارد شده باشند) از همین پیشنهاد کم می‌شوند تا عدد
 * پیشنهادی همان چیزی باشد که کاربر در صورت‌حساب بروکر می‌بیند.
 */
export function suggestPnl(opts: {
  ...
  // حجم بر حسب دلار یعنی کاربر خودش ارزش اسمی را داده، پس فقط درصد حرکت
  const gross =
    opts.volumeUnit === "USD"
  ...
  // سواپ می‌تواند مثبت هم باشد — پس با علامت خودش جمع می‌شود.
  const net = gross - Math.abs(opts.commission || 0) + (opts.swap || 0);
  return round2(net);
}

/** ریسک اولیه به ارز حساب — مخرج R؛ از فاصله‌ی ورود تا حدضرر */
...
/**
 * R = سود/زیان خالص ÷ ریسک اولیه.
 * اگر ریسک اولیه معلوم نباشد null برمی‌گردد — عمدا صفر نمی‌دهیم، چون
 * «معامله‌ای که R نداشت» نباید میانگین R را به سمت صفر بکشد.
 */
