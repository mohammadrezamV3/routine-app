...
- هیچ جلسه‌ای را صرفاً برای کوتاه کردن خروجی به چند حرکت محدود نکن.
...
پیش از طراحی، بررسی کن که درخواستِ کاربر (با توجه به توضیحی که خودش نوشته، محدودیت جسمی‌اش و روزهایی که
...
اگر واقع‌بینانه بود، فقط همین JSON خام را برگردان (بدون Markdown fence، بدون هیچ متن دیگر) — دقیقاً یک
آیتم به‌ازای هر روزی که کاربر گفته باشگاه می‌رود (نه کمتر، نه بیشتر)، و مقدارِ day باید دقیقاً یکی از
...
- name: نام فارسیِ رایجِ حرکت در بدنسازی ایران.
- muscle: عضله‌ی هدفِ اصلیِ همان حرکت.
- sets: عدد.
...
// تا بتواند واقعاً مثل یک مربی برنامه بنویسد. ولی کلِ اپ (جدولِ برنامه‌ی روز،
// ردیابِ ست‌به‌ست، فرمِ دستی، برنامه‌ی هفتگی) روی `items: string[]` با فرمتِ
// «نامِ حرکت ۴×۸» بنا شده؛ پس همین‌جا به همان رشته تخت می‌شود، به‌جای اینکه
// شکلِ داده‌ی کلِ اپ عوض شود. خروجیِ قدیمی (items به‌صورت رشته) هم هنوز
// پذیرفته می‌شود تا پاسخ‌های در-راهِ مدل ناگهان بی‌اعتبار نشوند.
function flattenExercise(ex: any): string | null {
  ...
  // «۳۰ ثانیه»/«۲ دقیقه» → حرکتِ زمان‌محور؛ عددِ خالی/بازه → ست×تکرار
