...
// می‌شه دور زد — کافیه کاربر مستقیم API رو صدا بزنه. این فایل همون منطق
// ModuleGate رو سمت سرور اجرا می‌کنه تا هیچ روت پولی بدون دسترسی واقعی
// اجرا نشه. منطق دقیقا آینه‌ی /api/account و ModuleGate است:
//   سوپریوزر → همیشه مجاز؛
//   وگرنه → باید یک ردیف moduleAccess فعال و منقضی‌نشده برای همون ماژول باشه.
...
/**
 * بررسی می‌کند کاربر واردشده به این ماژول دسترسی دارد یا نه.
...
export async function requireModule(module: ModuleKey): Promise<ModuleGuardResult> {
  ...
  // چون session با JWT ذخیره می‌شه (نه یک ردیف قابل‌ابطال سمت سرور)، اگه
  // Owner یک کاربر رو بعد از صدور توکن مسدود کنه، خود توکن هنوز «معتبر»
  // به‌نظر می‌رسه — پس این‌جا صریحا از دیتابیس چک می‌کنیم تا مسدودشدن واقعا
