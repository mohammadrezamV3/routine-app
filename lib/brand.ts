...
/**
 * هردو املا کنار هم — طبق درخواست صریح کاربر، این دیگه توی متنِ فارسیِ
 * قابل‌دیدن (H1، description، UI) استفاده نمی‌شه؛ برند فارسی همیشه فقط
 * «آریون» است. این export فقط برای جاهایی می‌مونه که ساختارا انگلیسی‌ان
 * (alternateName توی JSON-LD، یا یک خط خلاصه‌ی انگلیسیِ llms.txt).
 */
...
// برند اصلی همیشه خودِ «آریون»ه — نه «آریون اپ». در متنِ فارسی هیچ‌جا
// «(Arion)» کنارش نمی‌آد؛ عنوان و توضیح با خودِ «آریون» شروع می‌شن و بعد
// کل پلتفرم (روتین، ورزش، تغذیه، ترید، رودمپ) رو توصیف می‌کنن، نه فقط
// بخش روتین — چون آریون خیلی فراتر از یک «روتین اپ» تنهاست. «Arion»
// همچنان جایی که ساختارا انگلیسیه (alternateName توی JSON-LD، دامنه،
// شبکه‌های اجتماعی) هست — همون‌جاهایی که این قانون استثنا می‌کنه.
export const BRAND_TITLE = `${BRAND_FA} | برنامه‌ریزی روزانه، بدنسازی، کالری‌شمار و ژورنال ترید`;

// توجه: «رودمپ یادگیری» عمدا این‌جا نیست — لیستِ ماژول‌ها باید با چیزی که
// یک کاربر معمولی واقعا می‌بینه یکی باشه، و رودمپ فعلا پشتِ قفلِ
// requireSuperAdmin است (فقط سوپریوزر)، نه یک ماژولِ زنده برای همه؛
// گفتنش این‌جا به‌عنوان یک ویژگیِ در دسترس، توصیفِ غیرصادقانه‌ی محصول می‌شد.
export const BRAND_DESC =
  `${BRAND_FA} یک اپلیکیشن فارسی برای مدیریت زندگی روزمره است: روتین روزانه و هفتگی، ` +
  `پیگیری عادت‌ها و کارها، برنامه‌ی بدنسازی و کالری‌شماری با کمک هوش‌مصنوعی، و ژورنال معاملات ترید ` +
  `— همه زیر یک حساب کاربری.`;
...
//
// عمدا این‌جا «آریون اپ»/«اپ آریون»/«Arion App» نیست: طبق درخواست صریح
// کاربر، برند باید فقط «آریون / Arion» شناخته بشه، نه «آریون اپ» —
// نوشتن این ترکیب به‌عنوان alternateName یعنی به گوگل می‌گیم این هم یک
// نامِ رسمیِ برنده، که دقیقا برخلاف همون درخواسته.
export const BRAND_ALT_NAMES = [
  BRAND_FA, BRAND_EN, `${BRAND_EN} | ${BRAND_FA}`, `${BRAND_EN} ${BRAND_FA}`,
...
export const OG_IMAGE = { url: "/og.png", width: 1200, height: 630, alt: BRAND_FA };
