...
// ============================================================================
// V3 — Personal Baseline: میانگین شخصی کاربر روی هر دامنه (نه یک مقیاس
// عمومی)، + یک پیش‌بینی نرم برای هفته‌ی جاری. فرمول Score اصلا عوض
// نمی‌شه (algorithmVersion=1 می‌مونه) — این فقط یک لایه‌ی نمایشی اضافه‌ست.
// ============================================================================

export type Baseline = { domain: Domain; average: number; weeksConsidered: number };

/** میانگین N هفته‌ی اخیر (trailingWeeks) — نگاه کن به lib/weeklyReport/trailing.ts. */
export function computeBaselines(trailingWeeks: TrailingWeek[]): Baseline[] {
  ...
  for (const d of DOMAINS) {
    ...
    if (scores.length < 3) continue; // زیر ۳ هفته، baseline قابل‌اتکا نیست
...
/**
 * فقط برای هفته‌ی جاری معنا داره. شرط: میانگین روزهای‌تاکنون این هفته
 * به‌طور محسوس (≥۱۵ امتیاز) زیر baseline باشه، و Trend همون دامنه هم
 * نزولی باشه — یعنی هم «الان بد»، هم «روند اخیر هم بد». هیچ‌وقت قطعی.
 */
export function computePrediction(
  ...
  for (const baseline of baselines) {
    ...
    return {
      ...
      message: `بر اساس روند اخیر و میانگین شخصی‌ات (${baseline.average}٪)، این هفته فعلا ${metric.score}٪ه — احتمال یک هفته‌ی ضعیف‌تر از حد معمولت وجود داره.`,
      confidence: metric.daysWithData >= 4 ? "medium" : "low",
      evidence: `میانگین ${baseline.weeksConsidered} هفته‌ی اخیر: ${baseline.average}٪ — این هفته تا الان: ${metric.score}٪`,
    };
  }
  return null;
}
