...
// سقف انتظار برای پاسخ درگاه.
//
// چرا لازم است: `fetch` در Node **هیچ تایم‌اوت پیش‌فرضی ندارد** — تا ابد
...
// اتصال را می‌بندد و یک صفحه‌ی **HTML** با کد ۵۰۴ به مرورگر می‌دهد. آن‌وقت
// کلاینت که منتظر JSON است روی `res.json()` خطا می‌خورد و کاربر پیام
// گمراه‌کننده‌ی «مشکلی در اتصال به سرور پیش اومد» را می‌بیند — بدون هیچ
// سرنخی از این‌که در واقع درگاه دیر جواب داده.
//
// با این تایم‌اوت، خطا همیشه *داخل خود اپ* رخ می‌دهد و به‌صورت JSON با یک
// پیام فارسی دقیق برمی‌گردد. عدد از `proxy_read_timeout` کمتر است تا
...
export async function zibalVerify(trackId: number) {
  ...
  return {
    ok: data.result === 100 || data.result === 201, // ۲۰۱ = قبلا تایید شده (idempotent)
    result: data.result,
