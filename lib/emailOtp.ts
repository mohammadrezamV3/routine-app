...
// منطق مشترک کد OTP ایمیل — بین روت request، روت verify، و
// authorize() provider «email-otp» توی lib/auth.ts به اشتراک گذاشته می‌شه
// تا هش‌کردن/طول کد یک‌جا تعریف بشه، نه سه بار تکراری.

export const EMAIL_OTP_LENGTH = 6;
export const EMAIL_OTP_TTL_MS = 10 * 60 * 1000; // ۱۰ دقیقه
export const EMAIL_OTP_MAX_ATTEMPTS = 5;

/** تولید کد ۶رقمی با crypto.randomInt — امن رمزنگاری‌شده (نه Math.random) */
...
/** مقایسه‌ی زمان-ثابت (timing-safe) به‌جای === خام روی هش hex */
...
/**
 * بررسی نهایی + مصرف واقعی OTP (usedAt) — منبع واحد حقیقت «آیا این کد
 * الان معتبره و می‌شه باهاش وارد شد»، هم برای authorize() provider
 * «email-otp» توی lib/auth.ts (که واقعا نشست صادر می‌کنه) هم برای تست.
 * attempts فقط روی «کد غلط» زیاد می‌شه، نه روی نبودن/expired‌بودن ردیف —
 * تا کسی نتونه با چک‌کردن رفتار attempts بفهمه اصلا OTP فعالی برای این
