...
// چک‌کردنِ رمزِ کرانِ بیرونی (crontabِ خودِ VPS) — مشترک بینِ همه‌ی
// روت‌های cron (send-reminders، weekly-report). استخراج‌شده از
// app/api/push/send-reminders/route.ts تا این منطقِ امنیتی دو جای جدا
// نگهداری/دریفت نکنه.

/**
 * مقایسه‌ی رمز در زمانِ ثابت — `!==` معمولی به‌محضِ اولین بایتِ متفاوت
...
/** true یعنی درخواست رمزِ CRON_SECRET درستی داشته (هدرِ Bearer یا کوئریِ secret). */
