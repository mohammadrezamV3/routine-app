// اتصالِ متاتریدر — منطقِ مشترکِ کد/توکن و نرمال‌سازیِ معاملاتِ ورودی.
//
// مدلِ امنیتی (خلاصه، تا کسی موقعِ تغییر ندانسته نشکندش):
//   ۱) رمزِ حسابِ معاملاتی هیچ‌وقت از کاربر خواسته و هیچ‌جا ذخیره نمی‌شود.
...
//      همان را در تنظیماتِ EA می‌گذارد. EA یک‌بار آن را می‌فرستد و در عوض
//      یک توکنِ دائمی می‌گیرد؛ از آن به بعد فقط توکن رد و بدل می‌شود.
//   ۳) نه کد و نه توکن خام ذخیره نمی‌شوند — فقط SHA-256. چون هر دو رشته‌ی
//      تصادفیِ پرآنتروپی‌اند (نه رمزِ انتخابیِ انسان)، هشِ سریع درست است:
//      حمله‌ی دیکشنری روی ۱۶۰ بیت آنتروپی بی‌معناست، و برخلافِ bcrypt
//      اجازه‌ی جست‌وجوی مستقیم با ایندکس را هم می‌دهد.
//   ۴) توکن هر لحظه از پنل قابلِ ابطال است و ابطال فوری اثر می‌کند (چون
//      هر درخواست مستقیم به دیتابیس می‌خورد، نه به یک JWTِ امضاشده).

import { createHash, randomBytes } from "crypto";

export type MtPlatform = "MT4" | "MT5";

/** بدونِ حروف/ارقامِ شبیه‌به‌هم (0/O، 1/I) — کاربر باید این را دستی تایپ کند */
...
/** کدِ اتصال: ۱۲ کاراکتر از الفبای ۳۲تایی ≈ ۶۰ بیت آنتروپی */
...
/** توکنِ دائمیِ EA — ۳۲ بایت تصادفی (۲۵۶ بیت) */
...
// ── نرمال‌سازیِ معاملاتِ ارسالیِ EA ────────────────────────────────────────
...
/**
 * ردیف‌های خامِ EA را به شکلِ داخلی تبدیل می‌کند.
 * ردیفِ بدشکل بی‌صدا کنار گذاشته می‌شود، نه اینکه کلِ sync را بشکند — یک
 * نمادِ عجیبِ بروکر نباید باعث شود بقیه‌ی معاملات هم ثبت نشوند.
 */
export function normalizeMtTrades(raw: unknown): MtTradeInput[] {
  ...
  for (const item of raw.slice(0, 500)) {
    ...
    // MQL هم رشته می‌دهد هم عددِ نوعِ سفارش (۰ = buy، ۱ = sell)
