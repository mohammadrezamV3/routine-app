...
const MIN_GAP = 0.22;
// قبلا ۰٫۱۳ بود — آیتم‌هایی که ساعتشون خیلی نزدیک به بیداری/خواب بود
// می‌رفتن زیر لیبل همون نقطه‌ی انتهایی و روش overlap می‌کردن. فاصله‌ی
// بیشتر یعنی همیشه یه حداقل فضای امن بین اولین/آخرین آیتم و دو تا انتها.
const EDGE_GAP = 0.19;

/**
 * محاسبه موقعیت (pct بین ۰ و ۱) هر تسک زمان‌دار روی خط تایم‌لاین، طوری که
...
export function positionTimedTasks(
  ...
  if (n) {
    ...
    // پیش‌کلمپ: هر آیتم اول توی بازه‌ی مجاز [EDGE_GAP, 1-EDGE_GAP] جا می‌گیره —
    // بدون این، آیتم آخری که ساعتش خیلی نزدیک خوابه یه pct خام نزدیک ۱
    // داره که پاس رفت زیر رو رد می‌کنه و مجبور به «شیفت یکجای همه به عقب»
    // می‌شد؛ اون شیفت آیتم اول (نزدیک بیداری) رو زیر EDGE_GAP می‌فرستاد و
    // دقیقا باعث همون باگ گزارش‌شده می‌شد (افتادن روی کلمه‌ی «بیداری»).
    for (let i = 0; i < n; i++) {
      sorted[i].pct = Math.min(Math.max(sorted[i].pct, EDGE_GAP), 1 - EDGE_GAP);
    }
    // پاس رفت: هرکدوم حداقل gap بعد قبلی
    sorted[0].pct = Math.max(sorted[0].pct, EDGE_GAP);
    for (let i = 1; i < n; i++) sorted[i].pct = Math.max(sorted[i].pct, sorted[i - 1].pct + gap);
    // پاس برگشت: هرکدوم حداقل gap قبل بعدی — وقتی پاس رفت آخری رو از سقف
    // رد کرده بود (چند آیتم نزدیک هم ته لیست)
    ...
    // پاس رفت دوم: پاس برگشت ممکنه آیتم اول رو زیر EDGE_GAP برده باشه
    sorted[0].pct = Math.max(sorted[0].pct, EDGE_GAP);
    for (let i = 1; i < n; i++) sorted[i].pct = Math.max(sorted[i].pct, sorted[i - 1].pct + gap);
  }
