...
- تقویم اقتصادی همیشه از جدول خودمون خونده می‌شه، نه مستقیم از سرویس بیرونی (CSP اجازه نمی‌ده و نباید به یه سرویس خارجی گره بخوره). منبع با این اولویت انتخاب می‌شه: `ECONOMIC_CALENDAR_URL` (override دستی) → `ECONOMIC_CALENDAR_API_KEY` (JBlanked، هدر `Authorization: Api-Key`) → **فیدِ رایگانِ فارکس‌فکتوری بدونِ کلید** (سه فایل هفتگی lastweek/thisweek/nextweek). یعنی تقویم *هیچ‌وقت* بدون کار ادمین مرده نمی‌مونه — قبلا بدون کلید بی‌صدا خاموش می‌شد و نتیجه‌ش «actual هیچ‌وقت نمیاد» بود. `externalProviderName()` باید همیشه با همین اولویت هم‌نظر بمونه، چون `source` نیمی از کلید یکتای upsertه. کران روزانه با همون کلید upsert می‌کنه؛ `ECONOMIC_CALENDAR_URL` فقط برای عوض‌کردن کامل منبع به یه فید دیگه لازمه. نکته: تاریخ JBlanked فرمت `YYYY.MM.DD HH:mm:ss` (نه ISO) داره و به‌صورت UTC فرض می‌شه (`lib/economicCalendar.ts` → `parseCalendarDate`) — این فرض هنوز با یه رویداد واقعی تأیید نشده.
