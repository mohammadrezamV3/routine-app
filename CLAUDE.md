...
- تقویم اقتصادی همیشه از جدول خودمون خونده می‌شه، نه مستقیم از سرویس بیرونی (CSP اجازه نمی‌ده و نباید به یه سرویس خارجی گره بخوره). منبع پیش‌فرض Trading Economics (نه فارکس‌فکتوری) — نیازمند `ECONOMIC_CALENDAR_API_KEY`؛ بدونش sync خودکار خاموشه و فقط ورود دستی ادمین کار می‌کنه. کران روزانه با همون کلید upsert می‌کنه؛ `ECONOMIC_CALENDAR_URL` فقط برای عوض‌کردن کامل منبع به یه فید دیگه لازمه.
