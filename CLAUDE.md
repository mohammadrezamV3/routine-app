...
- تقویم اقتصادی همیشه از جدول خودمون خونده می‌شه، نه مستقیم از سرویس بیرونی (CSP اجازه نمی‌ده و نباید به یه سرویس خارجی گره بخوره). منبع پیش‌فرض JBlanked Calendar API (نه Trading Economics، نه فارکس‌فکتوری) — نیازمند `ECONOMIC_CALENDAR_API_KEY` (از jblanked.com/profile، هدر `Authorization: Api-Key`)؛ بدونش sync خودکار خاموشه و فقط ورود دستی ادمین کار می‌کنه. کران روزانه با همون کلید upsert می‌کنه؛ `ECONOMIC_CALENDAR_URL` فقط برای عوض‌کردن کامل منبع به یه فید دیگه لازمه. نکته: تاریخ JBlanked فرمت `YYYY.MM.DD HH:mm:ss` (نه ISO) داره و به‌صورت UTC فرض می‌شه (`lib/economicCalendar.ts` → `parseCalendarDate`) — این فرض هنوز با یه رویداد واقعی تأیید نشده.
