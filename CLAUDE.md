...
## قوانین ثابت
- **دیگه به `README.md` ورودی نسخه اضافه نکن** (درخواست صریح کاربر). قانون قبلی «هر تغییر قابل توجه یک خط جدید بالای README می‌گیره» لغو شد — نه ورودی جدید، نه شماره‌ی نسخه‌ی جدید. شرح تغییرات فقط توی پیام کامیت و بدنه‌ی PR می‌ره.
- هر بخش جدید رو بساز و تحویل بده، بدون توقف برای اجازه گرفتن — فقط تغییر رو بده و برو مرحله بعد.
- بعد از هر push به برنچ فیچر، همیشه یک Pull Request بساز (اگه از قبل باز نیست) — این صریحا درخواست کاربره، فراموش نشه.
- هیچ فیچری از roadmap بلندمدت رو خودسرانه شروع نکن مگه صریح خواسته بشه.
- persistence: برای مهمان (لاگین‌نکرده) → `localStorage`؛ برای کاربر لاگین‌کرده → API واقعی روی Prisma/Postgres. این سوییچ توی `lib/storage.ts` انجام می‌شه و بقیه‌ی کامپوننت‌ها اصلا نمی‌دونن داده از کجا میاد — این قرارداد رو نشکن.
- اتصال متاتریدر روی خود حساب می‌شینه نه کاربر (هر کاربر ده‌ها حساب داره). رمز حساب معاملاتی هیچ‌وقت خواسته/ذخیره نمی‌شه؛ کد اتصال و توکن EA فقط SHA-256 ذخیره می‌شن و معاملات با کلید یکتای `(accountId, externalId)` ضدتکرار می‌شن. فیلدهای دستی کاربر روی معامله‌ی همگام‌شده در sync بعدی دست نمی‌خورن.
- تقویم اقتصادی همیشه از جدول خودمون خونده می‌شه، نه مستقیم از سرویس بیرونی (CSP اجازه نمی‌ده و نباید به یه سرویس خارجی گره بخوره). منبع با این اولویت انتخاب می‌شه: `ECONOMIC_CALENDAR_URL` (override دستی) → `ECONOMIC_CALENDAR_API_KEY` (JBlanked، هدر `Authorization: Api-Key`) → **TradingView بدونِ کلید** (پیش‌فرض؛ `actual` داره) → اگه TradingView در دسترس نبود (فقط در پاسِ کامل، نه تند): **فیدِ رایگانِ فارکس‌فکتوری** (سه فایل هفتگی lastweek/thisweek/nextweek) به‌عنوانِ پشتیبان. علتِ عوض‌شدنِ پیش‌فرض از فارکس‌فکتوری به TradingView: فیدِ رایگانِ faireconomy اصلاً فیلدِ `actual` نداره — همون ریشه‌ی گزارشِ «actual هیچ‌وقت نمیاد» بود. یعنی تقویم *هیچ‌وقت* بدون کار ادمین مرده نمی‌مونه. `externalProviderName()` باید همیشه با همین اولویت هم‌نظر بمونه، چون `source` نیمی از کلید یکتای upsertه. کران روزانه با همون کلید upsert می‌کنه؛ `ECONOMIC_CALENDAR_URL` فقط برای عوض‌کردن کامل منبع به یه فید دیگه لازمه. جدا از کران، خودِ روتِ خواندنِ تقویم هم با `ensureFreshCalendar` داده‌ی کهنه رو خودکار تازه می‌کنه — پس تقویم دیگه به هیچ زمان‌بندِ بیرونی (crontab/cluster.js) وابسته نیست. نکته: تاریخ JBlanked فرمت `YYYY.MM.DD HH:mm:ss` (نه ISO) داره و به‌صورت UTC فرض می‌شه (`lib/economicCalendar.ts` → `parseCalendarDate`) — این فرض هنوز با یه رویداد واقعی تأیید نشده.
- ماژول ترید حساب‌محوره: هر معامله زیر یک `TradeAccount`ه، «حذف حساب» یعنی آرشیو (نه پاک‌کردن تاریخچه)، زمان معامله همیشه UTC ذخیره می‌شه و جلسه‌ی معاملاتی سمت سرور با DST واقعی حساب می‌شه (`lib/forexSessions.ts`). وضعیت چک‌لیست هر معامله در لحظه‌ی ثبت snapshot می‌شه، نه ارجاع زنده — این رو نشکن. و ناقص‌بودن چک‌لیست هیچ‌وقت نباید جلوی ثبت معامله رو بگیره.
- روشن/خاموش‌کردن قابلیت‌ها از پنل ادمین (`/admin/features`): فلگ‌ها در `AppSetting` با کلید `feature_flags` (`lib/featureFlags.ts` + `lib/featureFlagsServer.ts`)، سه حالت on/admins/off، Owner همیشه مجاز. هر روت API اون بخش باید `featureBlocked`/`requireFeature` رو چک کنه (نه فقط `FeatureGate`/`useFeature` کلاینتی) و فلگ جایگزین گیت ماژول پولی نیست، کنارشه. رودمپ دیگه با `requireSuperAdmin` بسته نیست، با فلگ `roadmaps` بسته‌ست.
...
- **هیچ‌وقت خودسرانه به هیچ عنصری بک‌گراند اضافه نکن** — نه به دکمه، نه به کارت، نه به هیچ‌چیز دیگه — مگر اینکه صریحا خواسته بشه. اگه یه عنصر باید بی‌بک‌گراند (transparent/ghost) بمونه، هر تغییر دیگه‌ای (مثل جابه‌جایی به یه ساختار DOM جدید، مثلا پورتال‌کردن) نباید باعث بشه یه قانون CSS دیگه (مثل ریست سراسری `button{}`) رو خودش بگیره — چک کن که بعد از هر تغییری، این عنصرها همچنان transparent بمونن.
...
- پیام خطای عمومی در ثبت‌نام (بدون افشای اینکه دقیقا شماره یا یوزرنیم تکراریه) — جلوگیری از User Enumeration.
...
- CSS injected/inline نباید camelCase property names داشته باشه (مثلا `backgroundColor` به‌جای `background-color` در استایل خام) — مرورگر silently نادیده می‌گیره و دیباگش سخته.
...
- استیت مشترک بین صفحات (weekly/home/day-modal/…) باید sync باشه — تغییر توی یکی باید فورا توی بقیه منعکس بشه.
