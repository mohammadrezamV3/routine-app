...
- اتصال متاتریدر روی خودِ حساب می‌شینه نه کاربر (هر کاربر ده‌ها حساب داره). رمزِ حسابِ معاملاتی هیچ‌وقت خواسته/ذخیره نمی‌شه؛ کدِ اتصال و توکنِ EA فقط SHA-256 ذخیره می‌شن و معاملات با کلیدِ یکتای `(accountId, externalId)` ضدتکرار می‌شن. فیلدهای دستیِ کاربر روی معامله‌ی همگام‌شده در sync بعدی دست نمی‌خورن.
- تقویم اقتصادی همیشه از جدولِ خودمون خونده می‌شه، نه مستقیم از سرویسِ بیرونی (CSP اجازه نمی‌ده و نباید به یه سرویسِ خارجی گره بخوره). بدونِ هیچ فیدی، با ورودِ دستیِ ادمین کار می‌کنه؛ با ست‌کردنِ `ECONOMIC_CALENDAR_URL` کرانِ روزانه upsert می‌کنه.
- ماژول ترید حساب‌محوره: هر معامله زیرِ یک `TradeAccount`ه، «حذفِ حساب» یعنی آرشیو (نه پاک‌کردنِ تاریخچه)، زمانِ معامله همیشه UTC ذخیره می‌شه و جلسه‌ی معاملاتی سمتِ سرور با DSTِ واقعی حساب می‌شه (`lib/forexSessions.ts`). وضعیتِ چک‌لیستِ هر معامله در لحظه‌ی ثبت snapshot می‌شه، نه ارجاعِ زنده — این رو نشکن. و ناقص‌بودنِ چک‌لیست هیچ‌وقت نباید جلوی ثبتِ معامله رو بگیره.
