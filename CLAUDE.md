...
- منطقِ مشترکِ وب/موبایل توی هسته‌ی مشترک می‌شینه، نه توی route: ساخت/regenerateِ رودمپ `lib/roadmapGeneration.ts`، خواندن + sync هنگامِ خواندنِ تقویم اقتصادی `lib/mobileTradeOnlineCalendar.ts`. تغییرِ رفتارِ وب رو همون‌جا بده تا اپ هم بگیره (و اگه شکلِ پاسخ عوض شد، قرارداد + کلاینتِ `mobile/` رو هم).
